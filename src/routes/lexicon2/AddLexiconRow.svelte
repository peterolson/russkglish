<script lang="ts">
	...
	import { partsOfSpeech, type LexiconEntry2 } from '@/data/lexicon.types';
	import { orthographyToIPA } from '@/lib/toIPA';

	export let isEditing = false;
	export let onCancel: () => void = () => {};
	export let initialEntry: LexiconEntry2 = {
		id: -1,
		orthography: '',
		enGloss: '',
		enCognate: '',
	...
	$: {
		...
		const existingRow = lexicon2.find((row) => ipa === orthographyToIPA(row.orthography) || row.orthography === orthography);
